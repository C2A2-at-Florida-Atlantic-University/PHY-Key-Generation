...
class LoadDatasetChannels():
    ...
    def load_iq_samples(self, file_path):
        ...
        #instance = f[self.instanceset_name][:]
        #instance = instance.astype(int)
        #instance = np.transpose(instance)

        #ids = f[self.idset_name][:]
        #ids = ids.astype(int)
        #ids = np.transpose(ids)
        
        '''ids_start = int(ids[0]) + 1
        ids_end = int(ids[-1]) + 1
        num_channels = ids_start - ids_end + 1
        num_pkt = len(ids)
        num_pkt_per_dev = int(num_pkt/num_channels)
        
        print('Dataset information: Channel ID ' + str(ids_start) + ' to Channel ID ' + 
              str(ids_end) + ', ' + str(num_pkt_per_dev) + ' packets per Channel.')'''
...
class ChannelIndSpectrogram():
    ...
    #CHANGE WINDOW AND OVERLAP LENGTHS 
    #INCREASE WINDOW AND DECREASE OVERLAP
    #CHECK STFT DOCUMENTATION ON HOW IT AFFECTS THE CHANNEL
    def _gen_single_channel_ind_spectrogram(self, sig, win_len=128, overlap=64):
        '''
